...
import { BASE_FEE, waitTransactionToBeConfirmedOrError, ZEROSECURE_PROGRAM_ID, } from "./utils/";
const ZERO_ADDRESS = "aleo1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq3ljyzc";
export function useCreateMultisigWallet({ feePrivate = true, waitToBeConfirmed = true, network = WalletAdapterNetwork.TestnetBeta, } = {}) {
     ...
     */
    const createMultisigWallet = async (multisigWallet) => {
        ...
        if (!multisigWallet.address) {
            ...
            let random_address = Address.from_private_key(PrivateKey.from_seed_unchecked(randomBytes));
        ...
        if (multisigWallet.threshold > multisigWallet.owners.length) {
            return setError(new Error("Threshold cannot be greater than the number of owners"));
        }
        //fill the owners array with the zero address
        while (multisigWallet.owners.length < 8) {
            multisigWallet.owners.push(ZERO_ADDRESS);
        }
        let transaction = new Transaction(publicKey, network, [
            {
